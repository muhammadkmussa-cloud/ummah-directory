...
async def test_register_duplicate_email(api_client):
    await api_client.post("/api/v1/auth/register", json={
        "email": "dup@example.org", "password": "TestPass1234!", "full_name": "Test",
    })
    response = await api_client.post("/api/v1/auth/register", json={
        "email": "dup@example.org", "password": "TestPass1234!", "full_name": "Test",
    })
...
async def test_login_success(api_client):
    await api_client.post("/api/v1/auth/register", json={
        "email": "login@example.org", "password": "TestPass1234!", "full_name": "Test",
    })
    response = await api_client.post("/api/v1/auth/login", json={
        "email": "login@example.org", "password": "TestPass1234!",
    })
...
async def test_login_wrong_password(api_client):
    await api_client.post("/api/v1/auth/register", json={
        "email": "wrong@example.org", "password": "TestPass1234!", "full_name": "Test",
    })
    response = await api_client.post("/api/v1/auth/login", json={
        "email": "wrong@example.org", "password": "WrongPass123",
    })
