...
async def test_mfa_setup_requires_password(api_client: AsyncClient, auth_headers: dict):
    resp = await api_client.post("/api/v1/mfa/setup", params={"password": "WrongPass1234!"}, headers=auth_headers)
...
async def test_mfa_setup_and_verify(api_client: AsyncClient, auth_headers: dict):
    ...
    setup_resp = await api_client.post("/api/v1/mfa/setup", params={"password": "StrongPass1234!"}, headers=headers)
    ...
    verify_resp = await api_client.post("/api/v1/mfa/verify", params={
    ...
    new_code = totp.now()
    disable_resp = await api_client.post("/api/v1/mfa/disable", params={
        "password": "StrongPass1234!",
        "code": new_code,
    }, headers=headers)
...
async def test_mfa_verify_invalid_code(api_client: AsyncClient, auth_headers: dict):
    resp = await api_client.post("/api/v1/mfa/verify", params={
        "password": "StrongPass1234!",
        "code": "000000",
    }, headers=auth_headers)
    assert resp.status_code == 400
