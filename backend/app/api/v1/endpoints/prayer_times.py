...
from app.schemas.prayer_times import PrayerTimeSettingsResponse, PrayerTimeSettingsUpdate

router = APIRouter()


@router.get("/me", response_model=PrayerTimeSettingsResponse)
async def get_my_prayer_time_settings(user: User = Depends(get_current_user)):
    """
