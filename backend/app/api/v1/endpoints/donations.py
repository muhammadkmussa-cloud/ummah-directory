...
def _generate_pdf_receipt(donation, charity_name: str, campaign_title: str | None) -> bytes:
    ...
    pdf.cell(
        0, 8, f"Receipt Number: {donation.receipt_number or 'N/A'}", new_x="LMARGIN", new_y="NEXT"
    )
    pdf.cell(
        0,
        8,
        f"Date: {donation.created_at.strftime('%Y-%m-%d %H:%M') if donation.created_at else 'N/A'}",
        new_x="LMARGIN",
        new_y="NEXT",
    )
    ...
    pdf.cell(
        0,
        8,
        f"Anonymous: {'Yes' if donation.is_anonymous else 'No'}",
        new_x="LMARGIN",
        new_y="NEXT",
    )
    pdf.ln(10)
    ...
    pdf.cell(
        0,
        5,
        "This receipt is auto-generated by Umma Directory.",
        align="C",
        new_x="LMARGIN",
        new_y="NEXT",
    )
...
async def initiate_donation(
    ...
    # Path 2: Direct donation to a verified organization
        ...
        org_uuid = (
            ...
            if isinstance(req.organization_id, str)
            else req.organization_id
        )
        ...
        if org.status != "approved":
            raise HTTPException(
                status_code=400, detail="Organization is not approved for donations"
            )
        organization_id = org.id
        org_name = org.name
    else:
        raise HTTPException(
            status_code=400, detail="Either campaign_id or organization_id is required"
        )
    ...
    if payment and payment.status == "succeeded":
        ...
        if donation.organization_id:
            org_result = await db.execute(
                select(Organization).where(Organization.id == donation.organization_id)
            )
        ...
        await create_notification(
            ...
            "donation.confirmed",
            "Donation confirmed",
            f"Your donation of {donation.amount} {donation.currency} to "
            f"{campaign.title if campaign else org_name} was successful.",
        )
...
async def download_receipt_pdf(
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": (
                f'attachment; filename="receipt-{donation.receipt_number or donation.id}.pdf"'
            )
        },
    )
...
async def donation_stats(
    ...
    total_q = await db.execute(
        select(func.count(), func.sum(Donation.amount)).where(Donation.status == "completed")
    )
    row = total_q.one()
    return {
