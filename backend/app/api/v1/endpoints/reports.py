...
ALLOWED_CATEGORIES = {"spam", "offensive", "incorrect", "duplicate", "other", "fraud", "closed", "scam"}
...
async def create_report(
    ...
    if category not in ALLOWED_CATEGORIES:
        raise HTTPException(status_code=400, detail=f"Invalid category. Must be one of: {ALLOWED_CATEGORIES}")
    ...
    db.add(report)
    await db.flush()
    await log_action(db, user.id, "report.create", resource_type, resource_id,
