...
async def search_all(
    ...
    cache_key = f"search:{q}:{type}:{category}:{city}:{verified}:{premier}:{min_rating}:{page}:{size}"
    cached = await cache_get(cache_key)
    if cached is not None:
        return cached

    results: dict[str, list[Any]] = {"businesses": [], "mosques": [], "charities": [], "events": [], "education": []}
    ...
    if not type or type == "business":
        ...
        if not q:
            active_featured_sq = select(AdCampaign.organization_id).where(
        ...
        for b in businesses:
            results["businesses"].append(BusinessResponse(
                ...
                logo_url=b.logo_url, cover_image_url=b.cover_image_url,
                avg_rating=b.avg_rating, review_count=b.review_count,
                is_verified=b.is_verified, is_premier=b.is_premier,
                is_featured=str(b.id) in featured_org_ids,
                status=b.status, category_id=str(b.category_id),
    ...
    if not type or type == "mosque":
        ...
        results["mosques"] = [{"id": str(m.id), "name": m.name, "slug": m.slug,
    ...
    if not type or type == "charity":
        ...
        results["charities"] = [{"id": str(c.id), "name": c.name, "slug": c.slug,
                                 "mission_statement": c.mission_statement,
                                 "city": c.city, "is_verified": c.is_verified}
    ...
    if not type or type == "education":
        ...
        results["education"] = [{"id": str(e.id), "name": e.name, "slug": e.slug,
                                 "institution_type": e.institution_type,
                                 "city": e.city, "is_verified": e.is_verified}
    ...
    if not type or type == "event":
        ...
        results["events"] = [{"id": str(e.id), "title": e.title, "slug": e.slug,
...
async def suggestions(
    ...
    results = []
    # Use ILIKE for quick suggestions since tsvector doesn't handle partial words well without prefix matching
    for model, name_attr, slug_attr, type_name in [
        ...
        (EducationalInstitution, EducationalInstitution.name, EducationalInstitution.slug, "education"),
    ]:
...
async def nearby_search(
    ...
    results: dict[str, list[dict]] = {"businesses": [], "mosques": [], "charities": [], "education": [], "events": []}
    offset_val = (page - 1) * size
    ...
    def st_distance(mlng, mlat):
        ...
        a = func.sin(dlat / 2) * func.sin(dlat / 2) + \
            func.cos(func.radians(mlat)) * func.cos(func.radians(lat)) * \
    ...
    if not type or type == "business":
        branch_sq = select(
            ...
            func.min(st_distance(BusinessBranch.longitude, BusinessBranch.latitude)).label("min_branch_dist")
        ).where(
            BusinessBranch.latitude.isnot(None),
            BusinessBranch.longitude.isnot(None)
        ).group_by(BusinessBranch.business_id).subquery()

        b_dist = st_distance(Business.longitude, Business.latitude)
        
        distance_m = func.least(
            func.coalesce(b_dist, 999999999), 
            func.coalesce(branch_sq.c.min_branch_dist, 999999999)
        )

        qb = select(Business, distance_m.label("distance")).outerjoin(
            branch_sq, branch_sq.c.business_id == Business.id
        ).where(
            Business.status == "approved",
            distance_m <= radius * 1000
        ).order_by(text("distance ASC")).offset(offset_val).limit(size)
    ...
    if not type or type == "mosque":
        qm = select(Mosque, st_distance(Mosque.longitude, Mosque.latitude).label("distance")).where(
    ...
    if not type or type == "charity":
        qch = select(Charity, st_distance(Charity.longitude, Charity.latitude).label("distance")).where(
    ...
    if not type or type == "education":
        qedu = select(EducationalInstitution, st_distance(EducationalInstitution.longitude, EducationalInstitution.latitude).label("distance")).where(
            ...
            st_distance(EducationalInstitution.longitude, EducationalInstitution.latitude) <= radius * 1000
        ).order_by(text("distance ASC")).offset(offset_val).limit(size)
        result = await db.execute(qedu)
        for row in result.all():
            e, dist = row[0], row[1]
    ...
    if not type or type == "event":
        qe = select(Event, st_distance(Event.longitude, Event.latitude).label("distance")).where(
